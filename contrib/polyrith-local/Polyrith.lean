...
open Lean
-- `Ring` was refactored into `Ring.Common` after this file was removed upstream.
open Meta Ring Ring.Common Qq PrettyPrinter AtomM
...
partial def parse {u : Level} {α : Q(Type u)} (sα : Q(CommSemiring $α))
    (c : Ring.Common.Cache sα) (e : Q($α)) : AtomM Poly := do
  let els := do
...
def sageHelperFunctions : String :=
  include_str "polyrith_helper.py"
...
/--
Runs the certificate computation locally, and parses the output as `SageResult`.

Upstream sent this query to a public SageMath server over `curl`; that server was shut down,
which is why the tactic was removed from Mathlib. The computation itself is small and
entirely local: `scripts/polyrith_local.py` feeds the query to Singular through
`passagemath-singular` and prints the same JSON envelope the service used to return, so
everything downstream of this function is unchanged.

The result is only a *suggestion*. It is turned into a `linear_combination` call which Lean
elaborates and checks, so an incorrect certificate cannot produce a theorem - it produces a
tactic failure.
-/
def runSage (trace : Bool) (α : Expr) (atoms : Nat) (hyps : Array (Source × Poly)) (target : Poly) :
  ...
  if exitCode != 0 && stdout.isEmpty then
    IO.throwServerError <|
      s!"Could not run the certificate backend ({python} {script}). " ++
      s!"It exited with code {exitCode}:\n{stderr}"

  -- parse results
  match Json.parse stdout >>= fromJson? with
  | .ok result => return result
  | .error e => IO.throwServerError <|
      s!"Could not parse the certificate output (error: {e})\nOutput:\n{stdout}"
